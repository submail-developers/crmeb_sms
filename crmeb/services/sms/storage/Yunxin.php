...
class Yunxin extends BaseSms
{
    ...
    public function send(string $phone, string $templateId, array $data = [])
    {
        ...
        $data['appid']  =   $this->smsAccount;
        $data['signature'] = $this->smsToken;
        $data['to'] = $phone;
        $template = $this->getTemplateCode($templateId);
        if (is_null($template)) {
            return $this->setError('Missing template number');
        }
        $tem = new RoutineModel();
        $content = $tem->where('tempid', $template)->find()['content'];
        foreach ($data as $key => $value) {
            $content = str_replace('{$' . "$key" . '}', $value, $content);
        }
        $data['content'] = $content;//要发送的短信内容
        ...
        if ($result['status'] == 'ok') {
            ...
            $ret['data']["template"] = $template;
            $ret['data']["id"] = $result['send_id'];
            return $ret;
        } else {
            return false;
        }
